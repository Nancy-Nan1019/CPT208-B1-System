document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadSpeakingLog() {
        ...
        apiRequest('/students/speaking-log?sessionId=' + group.sessionId).then(function (logs) {
            speakingTimeline = [];
            speakCountMap = {};
    ...
    function renderMembers(members) {
        ...
        container.innerHTML = members.map(function (member) {
            ...
            const turnsHtml = turns > 0 ? '<span style="color:var(--text-3);">' + turns + ' turn' + (turns > 1 ? 's' : '') + '</span>' : '';
            const tagHtml = '<div style="display:flex;align-items:center;gap:6px;font-size:12px;font-weight:500;margin-top:2px;">' +
                '<span style="color:var(--blue-600);background:rgba(59,130,246,0.1);padding:2px 6px;border-radius:4px;">' + memberPersonality + '</span>' +
                turnsHtml +
                '</div>';
            ...
            return '<div class="member-card" data-user-id="' + member.userId + '">' +
                ...
                '<span style="font-size:15px;color:var(--text-1);">' + memberName + '</span>' +
                tagHtml +
                '</div>' +
                '</div>' +
    ...
    function renderTimeline() {
        ...
        if (!speakingTimeline.length) {
            container.innerHTML = '<div class="form-hint">No speaking events yet.</div>';
        ...
        container.innerHTML = recent.map(function (entry) {
            ...
            return '<div class="avatar-line" style="padding:4px 0;">' +
                buildAvatarHtml(entry.userId, entry.name, null, 28) +
                '<div style="flex:1;min-width:0;">' +
                '<span style="font-weight:600;font-size:13px;">' + escapeText(entry.name) + '</span>' +
                '<span class="form-hint" style="margin-left:8px;">' + entry.duration + 's</span>' +
                '</div>' +
                '<span class="form-hint" style="flex-shrink:0;font-size:11px;">' + h + ':' + m + ':' + s + '</span>' +
                '</div>';
        }).join('');
    }
    ...
    function renderRanking(ranking) {
        ...
        container.innerHTML = ranking.map(function (member, index) {
            ...
            return '<div class="ranking-item' + rankClass + '">' + medal + escapeText(member.name || '') + ' - ' + (member.score || 0) + 's</div>';
        }).join('');
        applyStagger(container);
    }
